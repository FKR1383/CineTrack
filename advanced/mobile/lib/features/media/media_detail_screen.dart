...
class _MediaDetailScreenState extends ConsumerState<MediaDetailScreen> {
  ...
  Future<void> _reportComment(CommentModel comment) async {
    ...
    try {
      ...
      if (mounted) showMessage(context, 'گزارش برای مدیر سامانه ارسال شد.');
    } on ApiException catch (error) {
      if (mounted) showMessage(context, error.message, error: true);
    }
  }
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                _Score(label: 'کاربران Cine Track', value: media.appRating?.toStringAsFixed(1) ?? '—', scale: 5, subtitle: '${distribution.total} رأی'),
              ],
            ),
